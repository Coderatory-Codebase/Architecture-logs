...
🔴 Duplicate Ride Prevention — Partial Unique Index
Without this index, double-tap on Book button creates two REQUESTED rides. The partial unique index at database level guarantees one active ride per passenger — cannot be bypassed by app bugs or race conditions.
...
⚠️ GraphQL Endpoint — Decision Required
Base Server has a GraphQL endpoint for notifications. Doc 9 only uses Socket.io + FCM. Decision: Remove GraphQL endpoint — it adds complexity with no additional value when Socket + FCM covers all notification needs. Clean up in Base Server mapping phase.
 
